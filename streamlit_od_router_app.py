...
def create_combined_shapefile_zip(
    routes_gdf: gpd.GeoDataFrame,
    loaded_segments_gdf: gpd.GeoDataFrame,
    routes_name: str,
    loaded_name: str,
) -> bytes:
    """
    Creates one ZIP file containing BOTH shapefiles.
    """

    with tempfile.TemporaryDirectory() as temp_dir:

        routes_folder = os.path.join(temp_dir, "routes")
        os.makedirs(routes_folder, exist_ok=True)

        routes_shp = os.path.join(routes_folder, f"{routes_name}.shp")
        routes_gdf.to_file(routes_shp, driver="ESRI Shapefile")

        loaded_folder = os.path.join(temp_dir, "loaded_segments")
        os.makedirs(loaded_folder, exist_ok=True)

        loaded_shp = os.path.join(loaded_folder, f"{loaded_name}.shp")
        loaded_segments_gdf.to_file(loaded_shp, driver="ESRI Shapefile")
...
if submitted:
    if uploaded_file is None:
        ...
        try:
            ...
            combined_zip_bytes = create_combined_shapefile_zip(
                gdf,
                loaded_segments_gdf,
                f"{base_name}_{mode_input}",
                f"{base_name}_{mode_input}_loaded_segments",
            ...
            st.download_button(
                label="Download BOTH shapefiles in one ZIP",
                data=combined_zip_bytes,
                file_name=f"{base_name}_{mode_input}_ALL_SHAPEFILES.zip",
                mime="application/zip",
            )
